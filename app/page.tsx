...
export default function DatasetAnnotator() {
  ...
  const renderOverview = () => {
    const hasImages = currentDirImageCount > 0;
    const hasSubdirs = datasets.length > 0;
    const totalRecursive = datasets.reduce((acc, d) => acc + d.recursiveCount, 0) + currentDirImageCount;

    return (
      <div className="p-8 h-full flex flex-col bg-paper">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-5">
            {browsingHistory.length > 0 && (
              <button 
                onClick={handleGoBack}
                className="p-3 hover:bg-ink/5 rounded-2xl transition-all group border border-transparent hover:border-ink/10 active:scale-95"
                title="Back"
              >
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                <span className="text-[10px] text-ink/30 font-mono uppercase tracking-[0.2em]">
                ...
                {hasImages && (
                  <span className="text-[10px] text-brand font-bold uppercase tracking-widest">
                    {currentDirImageCount} Images found
                  </span>
                )}
              </div>
            </div>
          ...
          <button
            ...
            className="p-3 border border-ink/10 rounded-2xl hover:bg-ink/5 transition-all text-ink/40 hover:text-ink active:scale-95"
        ...
        <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar space-y-12">
          {/* LOAD ACTIONS SECTION */}
          {hasImages && (
            <section className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="bg-white border border-ink/5 rounded-[32px] p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold mb-1">Ready to annotate</h3>
                    <p className="text-sm text-ink/40 max-w-sm">
                      This folder contains {currentDirImageCount} images. You can load them directly or include everything from all subfolders.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                  <button
                    ...
                    className="px-8 py-4 bg-brand text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:shadow-xl hover:shadow-brand/20 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
                  >
                    <FolderOpen className="w-4 h-4" />
                    Load images from this folder ({currentDirImageCount})
                  </button>
                  {totalRecursive > currentDirImageCount && (
                    <button
                      onClick={() => browsingHandle && handleLoadDataset(browsingHandle, true)}
                      className="px-8 py-4 bg-ink text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:shadow-xl transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
                    >
                      <Layers className="w-4 h-4" />
                      Load everything from all folders ({totalRecursive})
                    </button>
                  )}
                </div>
              </div>
          ...
          {hasSubdirs ? (
            <section className="space-y-6">
              <div className="flex items-center justify-between px-2">
                <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-ink/30">Subdirectories</h4>
                <span className="text-[10px] font-mono text-ink/20">{datasets.length} Folders</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {datasets.map(dataset => (
                  <div 
                    ...
                    className="group relative bg-white border border-ink/5 rounded-[28px] overflow-hidden hover:border-brand/30 transition-all duration-500 hover:shadow-2xl cursor-pointer active:scale-[0.99]"
                  >
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="font-bold text-lg truncate pr-2 group-hover:text-brand transition-colors" title={dataset.name}>
                          {dataset.name}
                        </h3>
                        {dataset.hasSubdirs && (
                          <div className="flex items-center justify-center w-6 h-6 rounded-full bg-brand/5">
                      ...
                      <div className="flex items-center gap-5">
                        <div className="flex flex-col">
                          <span className="text-[8px] uppercase tracking-widest text-ink/30 font-bold mb-0.5">Images</span>
                          <span className="text-xs font-mono font-bold text-ink/60">{dataset.count}</span>
                        </div>
                        {dataset.recursiveCount > dataset.count && (
                          <div className="flex flex-col">
                            <span className="text-[8px] uppercase tracking-widest text-brand/40 font-bold mb-0.5">Total</span>
                            <span className="text-xs font-mono font-bold text-brand/60">{dataset.recursiveCount}</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div className="h-64 flex flex-col items-center justify-center text-ink/10 gap-6 border-2 border-dashed border-ink/5 rounded-[40px]">
              <FolderOpen className="w-16 h-16 opacity-10" />
              <div className="text-center space-y-1">
                <p className="font-mono text-xs uppercase tracking-widest">Empty directory</p>
                <p className="text-[10px] font-bold uppercase tracking-wider opacity-30">No images or subfolders found here</p>
              </div>
            </div>
          )}
        </div>
